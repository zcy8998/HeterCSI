...
% fc(GHz)  K   Δf(kHz) T  Δt(ms) UPA       Scenario          SpeedRange(km/h)
    ...
    {1.8,      64,  90,    24, 1,    [2,8],   '3GPP_38.901_Indoor_LOS', [0,10]};
    {2.1,      32,  120,   16, 0.5,  [4,8],   '3GPP_38.901_UMa_LOS', [30,120]};       
    {2.1,      32,  120,   24, 0.5,  [4,4],   '3GPP_38.901_RMa_LOS', [30,100]};
    {3.5,      32,  120,   16, 0.5,  [4,8],   '3GPP_38.901_UMa_NLOS', [30,100]};   
    {3.5,      64,  120,   16, 0.5,  [2,8],   '3GPP_38.901_UMi_LOS', [3,50]};    
    {6.7,      128,  180,   24, 1,    [2,4],   '3GPP_38.901_UMi_NLOS', [3,50]};     
    {6.7,      64,  120,   16, 1,    [4,4],   '3GPP_38.901_UMi_LOS', [0,10]};     
    {37.5,      128, 120,    24, 0.25,  [2,4],   '3GPP_38.901_RMa_NLOS', [3,50]};
    {37.5,      64,  240,    24, 0.125,  [4,4],   '3GPP_38.901_UMa_LOS', [30,100]};
...
% 创建输出目录
output_dir = '/Users/chenyuzhang/Desktop/csidata/zeroshot_extra';
...
for dataset_id = 1:length(datasets)
    ...
    disp(['Generating dataset D', num2str(12 +dataset_id), ' - ', scenario]);
    
    % 计算天线总数
    Ant = UPA(1) * UPA(2);
    
    % 创建数据集目录
    dataset_dir = fullfile(output_dir, sprintf('D%d', 12 +dataset_id));
    ...
    for sample_id = 1:samples_per_dataset
        ...
        if mod(sample_id, 1) == 0
            fprintf('Generated %d/%d test samples for D%d\n', sample_id, samples_per_dataset, 12 +dataset_id);
        end
    end
