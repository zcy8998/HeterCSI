...
def train_one_epoch_save_grads_v3(model,
                                  data_loader,
                                  device,
                                  epoch,
                                  args=None,
                                  ...
                                  collect_only=True):
    """
    Run logic to save per-sample gradients for selected batches.
    """
    model.eval()  # 固定参数，禁用 Dropout/BN 统计更新
    saved_count = 0
    
    # --- 1. 确定需要保存的 Batch 索引 (Deterministic Sampling) ---
    target_steps = None
    try:
        n_batches = len(data_loader)
        if n_batches > 0:
            seed_base = getattr(args, "seed", 0)
            seed = (seed_base if seed_base is not None else 0) + epoch
            rnd = random.Random(seed + epoch)
            n_to_sample = min(max_saved_batches, n_batches)
            target_steps = set(rnd.sample(range(n_batches), n_to_sample))
    except (TypeError, NotImplementedError):
        pass # 流式数据则按顺序保存

    # --- 2. 主循环 ---
    print(f"[Epoch {epoch}] Starting gradient collection...")
    
    for step, batch in enumerate(data_loader):
        # 退出条件
        if saved_count >= max_saved_batches:
            if collect_only:
                print(f"Collected {saved_count} batches; exiting early.")
                return
            break
        ...
        if grad_saver is not None and dataset_name is not None:
            try:                
                count, out_dir = grad_saver.save_per_sample_batch_grads(
                    model=model,
                    batch=batch,
                ...
                if count > 0:
                    saved_count += 1
                    print(f"[Per-sample grads] Saved batch #{saved_count} (step={step}) to {out_dir}")
            
            except Exception as e:
                print(f"[GradientSaver] Failed to save at step {step}: {e}")
                import traceback
                traceback.print_exc()

        # Debug stop
        if getattr(args, "debug_max_batches", None) and step >= int(args.debug_max_batches):
            break

    print(f"Finished epoch {epoch}. Collected {saved_count} batches.")
