...
def main(args):
    ...
    # args.resume="/data/zcy_data/CSIGPT/experiments/pretrain_csi_4gpu_128batch_doublemask_3mask/checkpoint-149.pth"
    pdb.set_trace()
...
def plot_prediction_error_heatmap(dataset, real_sample, pred_sample, mask_sample, seq_len, 
    ...
    参数:
    real_sample (np.ndarray): 真实样本数据，形状 [seq_len, feature_dim]
    pred_sample (np.ndarray): 预测样本数据，形状 [seq_len, feature_dim]
    mask_sample (np.ndarray): mask标记，布尔数组，形状 [seq_len]
    seq_len (int): 有效序列长度
    save_dir (str): 保存目录
    max_features (int): 最大可视化特征维度
    sample_idx (int): 样本索引（用于标题标注）
    timestamp (str): 时间戳（用于文件名）
    
    返回:
    str: 保存的文件路径（成功时）或错误信息
    """
    try:
        # 1. 验证输入
        if real_sample.shape != pred_sample.shape:
            raise ValueError(f"Shape mismatch: real {real_sample.shape} vs pred {pred_sample.shape}")
        
        # 2. 截取有效序列长度
        real_sample = real_sample[:seq_len]
        pred_sample = pred_sample[:seq_len]
        mask_sample = mask_sample[:seq_len]
        
        # 3. 计算绝对误差 (只保留被mask的位置)
        abs_error = np.abs(real_sample - pred_sample)
        abs_error[~mask_sample] = np.nan  # 未mask位置设为NaN
        
        # 4. 限制特征维度
        actual_features = min(max_features, abs_error.shape[1])
        error_vis = abs_error[:, :actual_features]
        
        # 5. 创建热力图
        plt.figure(figsize=(14, 8))
        ax = sns.heatmap(
            error_vis.T,  # 转置使特征在y轴
            cmap="viridis",
            cbar_kws={'label': 'Absolute Error'},
            mask=np.isnan(error_vis.T),
            xticklabels=max(1, seq_len // 20),  # 动态调整x轴刻度密度
            yticklabels=[f"Feature {i}" for i in range(actual_features)]
        )
        
        # 6. 优化布局
        plt.title(f"Prediction Error Heatmap (Sample #{sample_idx}, Seq Len: {seq_len})", fontsize=14)
        plt.xlabel("Time Steps", fontsize=12)
        plt.ylabel("Feature Dimensions", fontsize=12)
        plt.tight_layout()
        ...
        if timestamp is None:
            timestamp = time.strftime("%Y%m%d_%H%M%S")
        save_path = os.path.join("results", f"error_heatmap_sample_{dataset}_{sample_idx}_{timestamp}.png")
