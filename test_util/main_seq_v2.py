...
def main(args):
    ...
    for dataset_name in args.dataset.split(','):
        mmap_name = args.mask_type + '_seq_' + dataset_name
    ...
    # 在循环外部初始化结果记录文件
    if misc.is_main_process() and args.output_dir:
        results_file = os.path.join(args.output_dir, "validation_results.csv")
        with open(results_file, "w") as f:
            f.write("data_loader_idx,epoch,dataset_name,mask_type,nmse\n")

    dataset_name_list = args.dataset.split(',')
    for data_loader_idx, data_loader in enumerate(data_loaders, 1):
        for epoch in range(args.start_epoch, args.epochs):
            ...
            # 每25个epoch或最后epoch保存模型并验证
            if args.output_dir and ((epoch + 1) % 25 == 0):
                # 保存模型（添加数据集索引到文件名）
                model_path = f"checkpoint_d{data_loader_idx}_e{epoch}.pth"
                misc.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, 
                    optimizer=optimizer, loss_scaler=loss_scaler, epoch=epoch,
                    filename=model_path  # 需要修改save_model支持filename参数
                )
                if misc.is_main_process():
                    # 确定mask策略
                    mask_list = {'random': 0.85, 'temporal': 0.5, 'freq': 0.5} if args.mask_type == 'all' else {args.mask_type: args.mask_ratio}
                    
                    model.eval()
                    with torch.no_grad():
                        for mask_type, mask_ratio in mask_list.items():
                            for dataset_test in dataset_val_all:
                                dataset_name = dataset_test.dataset.get_dataset_name()
                                error_nmse = 0
                                num = 0
                                
                                for samples, token_length, input_size in dataset_test:
                                    samples = samples.to(device)
                                    token_length = token_length.to(device)
                                    
                                    # 前向计算
                                    loss, pred, mask = model(
                                        samples, token_length, input_size, 
                                    ...
                                    # 计算NMSE
                                    N, L, D = samples.shape
                                    mask_in_length = torch.arange(L, device=device)[None] < token_length[:, None]
                                    bool_mask = mask.bool()
                                    mask_nmse = bool_mask & mask_in_length
                                    
                                    y_pred = pred[mask_nmse].view(-1, D).detach().cpu().numpy()
                                    y_target = samples[mask_nmse].view(-1, D).detach().cpu().numpy()
                                    
                                    error_nmse += np.sum(np.linalg.norm(y_target - y_pred, axis=1)**2)
                                    error_nmse /= np.sum(np.linalg.norm(y_target, axis=1)**2)
                                    num += 1
                                
                                nmse = error_nmse / num if num > 0 else float('nan')
                                # data_loader_idx = dataset_name_list[data_loader_idx - 1]  
                                print(f"[D{data_loader_idx} E{epoch}] {dataset_name} {mask_type}: NMSE={nmse:.4f}")
                                
                                # 写入结果文件（CSV格式）
                                with open(results_file, "a") as f:
                                    f.write(f"D{data_loader_idx},{epoch},{dataset_name},{mask_type},{nmse:.6f}\n")
