    ...
    if misc.is_main_process():
        ...
        for task_idx, (mask_type, mask_ratio) in enumerate(mask_list.items()):
            for index, dataset_test in enumerate(dataset_val_all):
                ...
                print(f"Collecting features from {dataset_name}...")
                with torch.no_grad():
                    for iteration, (samples, token_length, input_size) in enumerate(dataset_test):
                        ...
                        aggregated_features = features.mean(dim=1) # [B, embed_dim]

                        # target_token_idx = 
                        # selected_features = features[:, target_token_idx, :]
                        # aggregated_features = selected_features

                        # 收集特征和标签
                        all_features.append(aggregated_features.cpu().numpy())
                        # 为当前批次的每个样本添加标签
                        all_labels.extend([dataset_name] * samples.shape[0])

        # 合并所有收集到的数据
        all_features = np.concatenate(all_features, axis=0)  # [Total_Samples, embed_dim]
        all_labels = np.array(all_labels)                    # [Total_Samples]

        print(f"Total samples collected: {all_features.shape[0]} from {len(np.unique(all_labels))} datasets.")

        # --- 第二阶段：统一进行PCA降维和3D可视化 ---
        ...
        ax.set_title('3D Feature Space Clustering of All CSI Scenarios', fontsize=16, fontweight='bold')
        ...
        # elev=20 表示从水平面“向上 20°”看模型（轻微俯视）

        # azim (azimuth)：方位角

        # 控制相机绕着 z 轴的旋转角度

        # azim=45 表示从 “右前方 45°” 看模型
        ax.view_init(elev=10, azim=75)

        plt.tight_layout()
        plt.savefig('csi_feature_clustering_3d_all_datasets.png', dpi=300, bbox_inches='tight')
